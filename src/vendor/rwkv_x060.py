...
class WKV_6(torch.autograd.Function):
    ...
    # patched: allow float32/bf16 for w
    # patched: allow float32/bf16 for u
            ...
            # patched: cast w/u to float32 for CUDA
            w = w.float().contiguous()
            u = u.float().contiguous() if u is not None else u
    ...
    def backward(ctx, gy): # backward: gy => gr, gk, gv, gw, gu
        with torch.no_grad():
            ...
            # patched: cast w/u to float32 for CUDA (backward)
            w = w.float().contiguous()
            u = u.float().contiguous() if u is not None else u
...
def RUN_CUDA_RWKV6(B, T, C, H, r, k, v, w, u=None):
    # original body (kept)
    # ensure w & u are float32 (C++ op expects float* for these)
    w = w.float().contiguous()
    if u is not None:
        u = u.float().contiguous()
    # call into the fused kernel
    w = w.float().contiguous()
    if u is not None:
        u = u.float().contiguous()
    w = w.float().contiguous()
...
tokenizer_path = os.getenv("RWKV_VOCAB_PATH", os.path.join(os.path.dirname(__file__), "rwkv_vocab_v20230424.txt"))
...
model_params = torch.load(os.getenv("RWKV_MODEL_PATH", r"S:\\Solomon\\models\\rwkv\\rwkv-final-v6-2.1-1b6.pth"), map_location="cpu")
